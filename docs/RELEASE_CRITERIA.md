...
| Trabalho e carreira                        | **Ausente**                  | Ocupação e renda inicial são dados do perfil, sem atividade diária, pagamento recorrente ou troca de ocupação ([estado](../src/core/state/state.ts), [arquétipos](../src/content/archetypes.ts)).                       | Implementar agenda de trabalho, pagamento mensal e duas transições de carreira.                          |
| Mudança de moradia                         | **Ausente**                  | A moradia é escolhida antes da campanha; não há ação de visita, mudança, caução ou novo contrato ([UI](../src/main.ts), [moradias](../src/content/housing.ts)).                                                         | Tornar as cinco opções comparáveis e selecionáveis durante o ano, com custo e deslocamento recalculados. |
| Calendário diário/mensal/anual             | **Ausente**                  | O relógio avança minutos e dias, e estudo usa ciclos de 30 dias, mas não há agenda, fechamento mensal nem conclusão do dia 365 ([economy](../src/core/economy/economy.ts), [education](../src/core/life/education.ts)). | Implementar loop diário, recorrências idempotentes, balanço mensal e epílogo anual.                      |
...
arquétipo, decisões, ciclos processados, saldo final e invariantes. Este gate é
**vermelho hoje**, pois o runner anual ainda não existe.
